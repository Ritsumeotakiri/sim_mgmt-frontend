...
import { apiRequest, apiRequestWithMeta } from '@/services/backendApi/client';
import { mapSim } from '@/services/backendApi/mappers';
...
export function SIMTable({ sims, userRole, onEdit, onDelete, onAdd, onSell, onBulkDelete, onBulkUpdateBranch, useServerPagination = false, selectedSimId = null, onOpenSimDetail, onCloseSimDetail }) {
  ...
  const [branches, setBranches] = useState([]);
    ...
    useEffect(() => {
      const loadBranches = async () => {
        ...
        try {
          const response = await apiRequestWithMeta(ENDPOINTS.branches.list);
    ...
    useEffect(() => {
      const loadServerSims = async () => {
        ...
        try {
          ...
          setServerPagination(response.pagination || { currentPage, pageSize, totalPages: 1, totalRecords: 0 });
        }
        finally {
          setLoading(false);
        }
      ...
      loadServerSims();
    }, [branchFilter, currentPage, searchTerm, statusFilter, useServerPagination, pageSize, sims]);
