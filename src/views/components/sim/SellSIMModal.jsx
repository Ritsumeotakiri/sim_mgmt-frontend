...
export function SellSIMModal({ isOpen, onClose, onSell, sim, availableMSISDNs, customers, plans, preselectedMSISDN = null, availableSIMs = [], lockedCustomer = null }) {
  ...
  const [selectedSIM, setSelectedSIM] = useState(sim || null);
    ...
    const handleSell = async () => {
      const resolvedSIM = (hasPreselectedMSISDN || isLockedCustomerFlow) ? selectedSIM : sim;
    ...
    return (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Summary */}
        {step === 3 && ((hasPreselectedMSISDN || isLockedCustomerFlow) ? selectedSIM : sim) && (hasPreselectedMSISDN ? preselectedMSISDN : selectedMSISDN) && selectedPlan && (<div className="mt-6 p-4 bg-[#f3f3f3] rounded-lg">
            <p className="text-sm font-medium text-[#1f1f1f] mb-2">Sale Summary</p>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-[#828282]">SIM ICCID:</span>
                <span className="font-mono">{(hasPreselectedMSISDN || isLockedCustomerFlow) ? selectedSIM?.iccid : sim?.iccid}</span>
              </div>
              <div className="flex justify-between">
