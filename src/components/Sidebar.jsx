...
import { LayoutDashboard, CreditCard, Phone, History, Users, Settings, ChevronLeft, ChevronRight, LogOut, User } from 'lucide-react';
...
export function Sidebar({ currentView, onViewChange, userRole, userName, onLogout }) {
    ...
    return (<aside className={cn("fixed left-0 top-0 h-screen bg-white border-r border-[#f3f3f3] transition-all duration-300 z-50", collapsed ? "w-20" : "w-64")}>
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between p-4 border-b border-[#f3f3f3]">
          ...
          {!collapsed && (<button onClick={() => setCollapsed(true)} className="p-1 hover:bg-[#f3f3f3] rounded-md transition-colors">
              <ChevronLeft className="w-4 h-4 text-[#828282]"/>
            </button>)}
          {collapsed && (<button onClick={() => setCollapsed(false)} className="absolute -right-3 top-6 w-6 h-6 bg-white border border-[#f3f3f3] rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-shadow">
