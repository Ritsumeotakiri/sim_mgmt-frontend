  ...
  useEffect(() => {
    const loadBranches = async () => {
      try {
        ...
        const validIds = restoredIds.filter((id) => data.some((b) => b.id === id));
      ...
      <div className="w-full space-y-6">
              ...
              <div className="absolute z-20 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg">
                ...
                <div className="max-h-72 overflow-y-auto p-1">
                  {filteredBranches.length === 0 ? (
                    <div className="text-sm text-gray-500 px-3 py-2">No branches found</div>
                  ) : (
        ...
        {selectedBranchIds.length > 0 ? (
              ...
              <div className="bg-white border border-gray-200 border-t-0 rounded-b-lg p-8 shadow-sm">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12">
                  ...
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-lg text-gray-800">Operators</h4>
                    ...
                    {activeBranch.operators?.length > 0 ? (
                      <ul className="space-y-3">
                        {activeBranch.operators.map((op) => (
                          <li
                            key={op.id}
                            className="bg-gray-50 rounded-lg p-4 flex items-center justify-between group hover:shadow-sm transition-shadow"
                          >
                      ...
                      <p className="text-gray-400 italic py-4">No operators assigned</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-lg text-gray-800">Managers</h4>
                      <span className="text-sm bg-gray-100 text-gray-600 px-3 py-1 rounded-full">
                    ...
                    {activeBranch.managers?.length > 0 ? (
                        ...
                        {activeBranch.managers.map((mgr) => (
                          ...
                          >
                            <div className="space-y-1">
                              ...
                              <p className="text-sm text-gray-500">{mgr.role}</p>
                      ...
                      <p className="text-gray-400 italic py-4">No managers assigned</p>
                    )}
                  </div>

                  {/* Performance Section with Chart */}
                  <div className="space-y-5">
                    <h4 className="font-semibold text-lg text-gray-800">Performance</h4>
                    <div className="bg-gray-50 rounded-lg p-5">
                      <div className="grid grid-cols-2 gap-6 mb-6">
                        <div>
                          <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Revenue</p>
                          <p className="text-2xl font-bold text-gray-900">
                            ${activeBranch.performance?.revenue?.toFixed(2) ?? '0.00'}
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">SIMs Sold</p>
                          <p className="text-2xl font-bold text-gray-900">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-16 text-center">
            ...
            <p className="text-gray-500 text-lg">No branches selected. Use the filter above to select branches.</p>
          </div>
        )}
      </div>
    </div>
