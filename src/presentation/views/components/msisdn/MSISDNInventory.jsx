...
export function MSISDNInventory({ msisdns, onAdd, onBatchImport, onEdit, onDelete, useServerPagination = false }) {
    ...
    const msisdnRequestRef = useRef(0);
    useEffect(() => {
      try {
      ...
      const loadBranches = async () => {
        ...
        try {
          ...
          while (hasNext) {
            ...
            nextBranches.push(...(response.data || []).map((branch) => ({ id: String(branch.id ?? branch.branch_id), name: branch.name })));
    ...
    useEffect(() => {
      const loadServerMsisdns = async () => {
        ...
        const requestId = ++msisdnRequestRef.current;
        try {
          setLoading(true);
          ...
          if (requestId !== msisdnRequestRef.current) {
            return;
          }
      ...
      <div className="bg-white rounded-xl border border-[#f3f3f3] shadow-sm">
        ...
        <div className="p-4 border-t border-[#f3f3f3] flex items-center justify-between">
          <p className="text-sm text-[#828282]">
            Showing {filteredMSISDNs.length === 0 ? 0 : startIndex + 1}-{Math.min(startIndex + paginatedMSISDNs.length, useServerPagination ? (serverPagination.totalRecords || filteredMSISDNs.length) : filteredMSISDNs.length)} of {useServerPagination ? (serverPagination.totalRecords || filteredMSISDNs.length) : filteredMSISDNs.length} MSISDNs
          </p>
